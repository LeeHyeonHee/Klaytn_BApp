...
const App = {
  ...
  submitAnswer: async function () {
    ...
    if (answer === result) {
      if (confirm("Good! 0.1 KLAY 받기")) {
        if (await this.callContractBalance() >= 0.1) {
          this.receiveKlay();
        } else {
          alert("죄송합니다. 컨트랙트의 KLAY가 소진되었습니다.");
        }
      }
    } else {
      alert("땡!");
  ...
  deposit: async function () {
    ...
    let owner = await this.callOwner();
    if (walletInstance) {
      if (owner.toLowerCase() !== walletInstance.address) return;
      else {
  ...
  checkValidKeystore: function (keystore) {
    ...
    const isValidKeystore = parseKeystore.version && parseKeystore.id && parseKeystore.address && parseKeystore.keyring;
  ...
  changeUI: async function (walletInstance) {
    ...
    if (owner.toLowerCase() === walletInstance.address) {
      $('#owner').show();
    }
  },
  ...
  receiveKlay: function () {
    ...
    agContract.methods.transfer(cav.utils.toPeb("0.1", "KLAY")).send({
      ...
      if (receipt.status) {
        ...
        $('#transaction').html("");
        $('#transaction')
        .append(`<p><a href='https://baobab.klaytnscope.com/tx/${receipt.senderTxHash}' target='_blank'>
                클레이튼 Scope에서 트랜젝션 확인</a></p>`);
          console.log(receipt);
